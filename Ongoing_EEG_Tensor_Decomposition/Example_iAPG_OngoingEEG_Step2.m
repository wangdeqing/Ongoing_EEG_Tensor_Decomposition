...
for jj=1:size(CORR1_P,2)
    for ii=1:size(CORR1_P,1)
        if CORR1_P(ii,jj)==1
            ...
            colorbar
            title(['Topograph#' int2str(ii)],'fontsize',18);
            % Waveform
            ...
            title(['Waveform, ' 'p=' num2str(p05(jj))  ', CC=' num2str(CORR1(ii,jj))],'fontsize',18);
            sLegend1=sprintf('Temporal Component #%d',ii);
            legend(sLegend1,MusicFeatureName{jj},'Location','best');
            % Spectrum
            subplot(3,4,[3 4 7 8])
